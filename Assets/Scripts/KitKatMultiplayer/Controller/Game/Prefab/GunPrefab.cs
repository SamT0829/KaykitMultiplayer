...
{
    [SerializeField] private BulletPrefab gameBulletPrefab;
    ...
    {
        var gameBullet = Instantiate(gameBulletPrefab, BulletSpawnPoint.position + aimForwardVector * 1, Quaternion.LookRotation(aimForwardVector));
        gameBullet.InitBullet(gamePlayerPrefab.GamePlayerRoomInfo.AccountId, bulletSpeed, BulletDamage);
        StartCoroutine(SpawnTrail(gameBullet, aimForwardVector));
        return gameBullet;
    }
